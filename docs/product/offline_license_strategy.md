...
디바이스(기기)는 **허가 또는 차단**의 이진 상태로 관리한다. 만료일 기반 관리를 하지 않는 이유:
...
- Platform에서 패키지를 만들 때 결정 (예: export 시점 + 30일)
...
`.ige`에 포함되는 사용자는 각각 소속과 만료일이 다를 수 있다. Platform은 export 시 **선택된 사용자 각각에 대해 `valid_until`을 개별 계산**하여 `users.json`에 넣는다.

```
선택된 사용자 각각에 대해:

  사용자가 조직 소속인 경우:
    valid_until = min(export 시점 + 기본 유효기간, 소속 조직 라이선스 만료일)

  사용자가 조직 미소속인 경우:
    valid_until = min(export 시점 + 기본 유효기간, 사용자 개인 만료일)
```

즉, **각 사용자의 상위 만료일로 cap**하는 동일한 로직이다. 소속 조직이 있으면 조직 만료일, 없으면 개인 만료일이 상한이 된다.

예시 — 같은 .ige에 3명의 사용자가 포함된 경우:

| 사용자 | 소속 | 상위 만료일 | 기본 유효기간 (30일) | 결과 valid_until |
|--------|------|-------------|---------------------|-----------------|
| 사용자 A | 조직 X (만료 2026-06-30) | 2026-06-30 | 2026-05-01 | **2026-05-01** |
| 사용자 B | 조직 Y (만료 2026-04-15) | 2026-04-15 | 2026-05-01 | **2026-04-15** |
| 사용자 C | 미소속 (개인 만료 2026-08-01) | 2026-08-01 | 2026-05-01 | **2026-05-01** |

Edge에서 로그인 시 **해당 사용자의 `valid_until`**만 확인한다. 사용자 B가 사용자 A보다 먼저 만료되어도 사용자 A의 접근에는 영향 없다.

오프라인 환경에서도 각 사용자의 상위 만료일을 넘겨서 사용할 수 없다. 만료 후 계속 사용하려면 해당 사용자의 라이선스를 갱신한 뒤 새 `.ige` 패키지를 재발급해야 한다.

#### 조직 만료일 변경 대응

조직 만료일이 단축될 수 있다 (예: 구독 미갱신). 이미 발급된 .ige의 사용자 `valid_until`이 변경 전 기준으로 설정되어 있으면, 조직은 만료됐는데 .ige 사용자는 아직 유효한 상황이 발생한다.

이 경우 **.ige를 재발급**하면 된다. 재발급 시 변경된 조직 만료일로 `valid_until`이 재계산된다. 별도의 `license_expires_at` 필드는 두지 않는다 — `valid_until` 하나로 만료를 관리한다.

**.ige users.json 사용자별 만료 정보:**
...
```
1. 해당 사용자의 valid_until 확인 → 지남? → "계정이 만료되었습니다. 새 패키지를 받으세요." 거부
2. 유효 → 로그인 허용
```

같은 .ige를 사용하는 사용자들이 각각 다른 시점에 만료된다. 조직 만료일이 변경되면 .ige를 재발급하여 반영한다.
...
**핵심**: 사용자가 아무것도 하지 않아도 자동 전환된다. 로그인 세션이 유지되고, 캐시된 만료일까지 작업을 계속할 수 있다.
...
1. 등록된 온라인 디바이스 목록에서 해당 기기 선택
2. "오프라인 전환" 클릭
   → 모드 변경: online → offline
   → fingerprint는 이미 등록되어 있으므로 재입력 불필요
3. license key 발급 (기존 fingerprint 기반)
4. .ige 발급 (fingerprint 포함)
5. Edge에 .ige 전달 → license key 입력 → 오프라인 운영 시작
```
...
- `valid_until`: 사용자 만료일 (상위 만료일로 cap 된 값). 이 날짜까지 오프라인 로그인 허용.
...
1. Platform에서 .ige export (기기명 입력)
   ...
   → 사용자별 만료일 = min(유효기간, 상위 만료일) 개별 계산
   → .ige 생성
         ↓
2. Edge에 .ige 전달 및 설치
...
[Export History — 기기별 관리]
  Device Name       | Version | Status    | Created    | Actions
  공장 A - 1번 라인 ✎ | v2      | completed | 2026-03-31 | [Download] [Reissue] [Delete]
  공장 B - 검수 라인 ✎ | v1      | completed | 2026-03-28 | [Download] [Reissue] [Delete]
```
...
- Export 폼에 "Device Name" 필드 추가
- Export 시 auth-service에 PENDING 디바이스 등록 API 호출
- `valid_until = min(유효기간, 조직 만료일)` cap 규칙 구현
...
- 사용자별 `valid_until` 개별 계산 로직 구현 (소속 조직 만료일 또는 개인 만료일로 cap)
